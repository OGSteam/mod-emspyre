...
$user_data["user_stat_name"] = $user_stat_name;
		...
		<table>
			...
			<tr>
				...
				<th rowspan="2"><U>intervalle d'étude</U> : du <input type="text"
					...
					value="<?php echo strftime("%d/%m/%Y", $max_date); ?>" /> </th>
				<th rowspan="2">
				</th>
			</tr>
			<tr>
...
// autrement on affiche rien : on prepare juste l affichage du script
    ...
    $pie_point = create_pie($b . "_x_" . $d . "_x_" . $l . "_x_" . $f . "_x_" . $t,
        "Batiments_x_Defenses_x_Lunes_x_Flotte_x_Technologies",
        "Derniere repartition des points connue", "pie_point");
}
echo "</div>";
echo "</th>\n";
...
// autrement on affiche rien : on prepare juste l affichage du script
    ...
    $pie_empire = create_pie(implode($planet, "_x_"), implode($planet_name, "_x_"),
        "Proportion des planetes - lunes comprises", "pie_empire");
}
